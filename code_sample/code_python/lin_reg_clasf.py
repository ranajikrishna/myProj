...
def reg_least_square(Z, y, lamda):
	...
	w_reg = np.dot(np.dot(LA.inv(w_reg_tmp), Z).T, y.values)		# Regression weights.	
...
def main(argv = None):
	...
	for v_int in range(0,10):
		...
		# --- In-sample data ---
		Z = pd.DataFrame(np.ndarray((data_train.shape[0], 3), dtype = 'd'))	 # In-sample data.  
		...
		# --- Out-of-sample data ---
		Y = pd.DataFrame(np.ndarray((data_test.shape[0], 3), dtype = 'd'))	 # Out-of-sample data.
	...
	for v_int in range(0,10):
		...
		# --- In-sample data ---	
		X = pd.DataFrame(np.ndarray((data_train.shape[0], 6), dtype = 'd'))	 # In sample data.
		...
		# --- Out-of-sample data ---
		A = pd.DataFrame(np.ndarray((data_test.shape[0], 6), dtype = 'd'))	 # Out-sample data.
