...
colnames(store_prb) <- c("x1_value","x2_value","y_value","classify","verify");
...
colnames(pts) <- c("x1_value","x2_value","y_value","classify");

# --- Generate the sample data pts. --- 
pts$x1_value <- runif(no_pts,-1,1);
pts$x2_value <- runif(no_pts,-1,1);
# Assign signs (+ve above the line).
pts$y_value <- sign(pts$x2_value - (fit$coefficients[2] * pts$x1_value + fit$coefficients[1]));
...
for (j in 1: tot_itr){
  ...
  store_prb$y_value[j] <- sign(store_prb$x2_value[j] - (fit$coefficients[2] * store_prb$x1_value[j] + fit$coefficients[1])); # Assign Sign (+ve above).
  store_prb$classify[j] <- sign(c(1,store_prb$x1_value[j],store_prb$x2_value[j]) %*% w); # Estimate Sign.
