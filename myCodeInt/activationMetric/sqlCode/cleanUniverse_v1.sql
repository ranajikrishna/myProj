...
(8) avg_mrr - average mrr at the end of the 2nd billing cycle. 
...
*/
select app_id, sum(cast(abs(duration_in_seconds) as real)) as tot_dur, avg_user, avg_msgs, avg_cmts, avg_mails, avg_admins, avg_mrr, sub_trial_st, trial_ed, sessions_app_id from ( -- t11

/* --- Join with Sessions --- */
select t10.app_id, avg_user, avg_msgs, avg_cmts, avg_mails, avg_admins, avg_mrr, sub_trial_st, 
trial_ed, duration_in_seconds, sessions.app_id as sessions_app_id from( -- t10
...
select distinct app_id, round(avg(cast(daily_users as real)),2) as avg_user, round(avg(cast(daily_messages as double precision)),2) as avg_msgs, round(avg(cast(daily_comments as double precision)), 2) as avg_cmts, round(avg(cast(daily_emails as double precision)),2) as avg_mails, round(avg(cast(admins as double precision)),2) as avg_admins, round(avg(cast(tot_mrr as double precision)),2) as avg_mrr, sub_trial_st, trial_ed from ( -- t6
...
select app_cust_id, app_id, app_name, min(subs_trial_st) as sub_trial_st, to_char(to_date(min(subs_trial_st),'YYYYmmdd') + interval '14 days', 'YYYYmmdd') as trial_ed, avg(datediff(days,to_date(app_created_at,'YYYYmmdd'), to_date(subs_trial_st,'YYYYmmdd'))) as time_to_trial, sum(mrr_amount) as tot_mrr from ( -- t3
...
/* Notes: 
(1) The mrr_amount id taken from the billin_invoices table, up to the 2nd billing cycle.
An upper bound on the mrr_amount of $10,000 is applied.
*/
select * from ( -- t1 / t2
select billing_subscriptions.id as subs_id, customer_id as subs_cust_id, to_char(billing_subscriptions.created_at, 'YYYYmmdd') as subs_created_at, to_char(billing_subscriptions.cancelled_at, 'YYYYmmdd') as subs_cancelled_at, to_char(billing_subscriptions.started_at, 'YYYYmmdd') as subs_trial_st, billing_invoices.id as invo_id, to_char(billing_invoices.period_end, 'YYYYmmdd') as invo_issue_at, billing_invoices.billing_cycle_number as cycle_number, billing_invoices.total_amount, billing_invoices.mrr_amount, billing_invoices.subscription_id as inv_subs_id
from billing_subscriptions  
left join billing_invoices
on billing_subscriptions.id = billing_invoices.subscription_id where cycle_number <= 2 and mrr_amount <= 1000000
) as t1 
...
) as t3 group by app_cust_id, app_id, app_name
) as t4
...
) as t6 group by app_id, sub_trial_st, trial_ed
) as t7
...
) as t11 group by app_id, avg_user, avg_msgs, avg_cmts, avg_mails, avg_admins, avg_mrr, sub_trial_st, trial_ed, sessions_app_id 
